...
依赖安装失败时，ABS 会显示简短原因、修复提示和日志路径，然后继续生成不完整结果。交互式终端中，ABS 会把 `sudo` 连接到 `/dev/tty`，因此 `curl | bash` 也可以正常询问 sudo 密码；无终端运行则使用非交互 sudo 并快速失败。ABS 不会自行运行 `dpkg --configure -a` 等系统修复命令。
